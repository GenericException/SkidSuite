...
public class Shrinker {
	 ...
	 * Properties properties = new Properties(); int n = args.length - 1; for
	 * (int i = 0; i < n - 1; ++i) { properties.load(new
	 * FileInputStream(args[i])); } for (Map.Entry<Object, Object> entry :
	 * properties.entrySet()) { MAPPING.put((String) entry.getKey(), (String)
	 * entry.getValue()); } final Set<String> unused = new
	 * HashSet<String>(MAPPING.keySet()); File f = new File(args[n - 1]); File d
	 * = new File(args[n]); optimize(f, d, new SimpleRemapper(MAPPING) {
	 * 
	 * @Override public String map(String key) { String s = super.map(key); if
	 * (s != null) { unused.remove(key); } return s; } }); Iterator<String> i =
	 * unused.iterator(); while (i.hasNext()) { String s = i.next(); if
	 * (!s.endsWith("/remove")) { System.out.println( "INFO: unused mapping " +
	 * s); } } }
	 */

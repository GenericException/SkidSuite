...
public class AntiStringer extends AntiBase {
				...
				if (isStringerDesc(desc)) {
					...
					if (callProxy) {
						if (callerCaller == null) {
							callerCaller = findCaller(methodHost, method);
							if (callerCaller == null) {
								return;
							}
						}
	...
	private String findCaller(ClassNode methodHost, MethodNode method) {
		for (ClassNode cn : getNodes().values()) {
			for (MethodNode mn : cn.methods) {
				for (AbstractInsnNode ain : mn.instructions.toArray()) {
					if (ain.getType() == AbstractInsnNode.METHOD_INSN) {
						MethodInsnNode min = (MethodInsnNode) ain;
						if (min.owner.equals(methodHost.name) && min.name.equals(method.name) && min.desc.equals(method.desc)) {
							return cn.name;
						}
					}
				}
